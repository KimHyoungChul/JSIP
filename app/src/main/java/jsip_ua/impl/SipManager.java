...
public class SipManager implements SipListener, ISipManager, Serializable {
	enum CallDirection { NONE, INCOMING, OUTGOING, };//枚举类型，在干嘛?方向吗
	...
	{
		sipManagerState = SipManagerState.REGISTERING;//初始化的时候状态是在注册
		this.sipProfile.setLocalIp(getIPAddress(true));//更改sipProfile里面的localIP地址
		...
		properties.setProperty("android.javax.sip.OUTBOUND_PROXY", sipProfile.getRemoteEndpoint()
				+ "/" + sipProfile.getTransport());//remote的ip地址加端口号，传输的协议类型，udp
		properties.setProperty("android.javax.sip.STACK_NAME", "androidSip");//协议栈的名字是一定要设置的
		...
		try {
			...
			sipStack = sipFactory.createSipStack(properties);//前面是初始化协议栈的各种属性，在这里创建一个协议栈
		...
		try {
			...
			udpListeningPoint = sipStack.createListeningPoint(
					sipProfile.getLocalIp(), sipProfile.getLocalPort(), sipProfile.getTransport());//本地的listening point
			sipProvider = sipStack.createSipProvider(udpListeningPoint);//这个接口描述了sipStack的消息实体
			sipProvider.addSipListener(this);
			initialized = true;
			sipManagerState = SipManagerState.READY;//sipManager的状态改变了，在这里变成了Ready
		} catch (PeerUnavailableException e) {
			return false;
		} catch (Exception e) {
			return false;
	...
	public synchronized void addSipEventListener(ISipEventListener listener) {//synchronized
		//这是一个公共资源，有可能发生访问冲突，所以要synchronize一下
	...
	public void AcceptCall(final int port) {//打电话有关的，暂时忽略
...
//			Request request = register.buildRegister(this);
	...
	@Override
	public void Call(String to, int localRtpPort)//不看
...
//			Request r = inviteRequest.buildMessage(this, to, message);
	...
	@Override
	public void Hangup() throws NotInitializedException//不看
	{
		if (!initialized)
			throw new NotInitializedException("Sip Stack not initialized");
